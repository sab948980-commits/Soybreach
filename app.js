...
async function nav(){
  ...
  n.innerHTML='<a href="index.html">threads</a><a href="search.html">search</a><a href="upload.html">new thread</a><a href="profile.html">profile</a>'+
    (u?'<a href="#" id="logoutLink">Logout ('+esc(p?.username||u.email)+')</a>':'<a href="login.html">Login</a>');
...
function card(p){
  ...
  return '<article class="card"><a href="post.html?id='+p.id+'">'+(p.image_url?'<img class="post-image" src="'+esc(p.image_url)+'">':'<div class="preview">'+esc(p.body)+'</div>')+'</a><div class="cardbody"><h3><a href="post.html?id='+p.id+'">'+esc(p.title)+'</a></h3><p class="muted">by '+esc(p.profiles?.username||'Unknown')+'</p>'+tags(ts)+'</div></article>';
}
...
async function setupUpload(){
  ...
  f.onsubmit=async e=>{
    ...
    try{
      ...
      const {data:post,error:pe}=await sb.from('posts').insert({user_id:u.id,title,body,image_url}).select('id').single();if(pe)throw pe;
...
async function renderPost(){
  ...
  el.innerHTML='<section class="panel"><h1>'+esc(p.title)+'</h1><p class="muted">by '+esc(p.profiles?.username||'Unknown')+' · '+new Date(p.created_at).toLocaleString()+'</p>'+(p.image_url?'<img class="post-image" src="'+esc(p.image_url)+'">':'')+
  ...
  (u&&u.id===p.user_id?' <button class="danger" onclick="deletePost('+p.id+')">Delete</button>':'')+'</p><hr><h2>Replies ('+(c?.length||0)+')</h2><div>'+
  ...
  '</div>'+(u?'<form id="commentForm"><label>Reply<textarea id="commentBody" rows="4" required></textarea></label><button>Reply</button></form>':'<p class="muted">Log in to reply.</p>')+'</section>';
...
async function deletePost(id){const u=await getUser();if(!u||!confirm('Delete this post?'))return;const {error}=await sb.from('posts').delete().eq('id',id).eq('user_id',u.id);if(error)alert(error.message);else location.href='index.html'}
async function setupRegister(){
  await nav();document.querySelector('#registerForm').onsubmit=async e=>{
    ...
    notice(data.session?'Account created!':'Account created. Check your email to confirm, then log in.');
...
async function renderProfile(){
  ...
  document.querySelector('#profile').innerHTML='<section class="panel profile"><div class="avatar">'+esc(p.username.slice(0,2).toUpperCase())+'</div><div><h1>'+esc(p.username)+'</h1><p class="muted">'+(a?.length||0)+' post'+((a?.length||0)===1?'':'s')+'</p></div></section><h2>Uploads</h2><div id="posts" class="grid"></div><p id="empty" class="muted"></p>';render(a||[],'#posts')
}
